...
def run_fio(hw_, disks_list, mode, io_size, time, rampup_time):
    ...
    for engine in ['libaio', 'io_uring']:
        # Let's check if fio supports the required engines
        if subprocess.call(['fio', '--enghelp={}'.format(engine)]) != 0:
            print('fio engine {} is not available'.format(engine))
            continue

        fio = 'fio --ioengine={} --invalidate=1 --ramp_time={} --iodepth=32 ' \
              '--runtime={} --time_based --direct=1 ' \
              '--bs={} --rw={}'.format(engine, rampup_time,
                                       time, io_size, mode)
        ...
        for disk in disks_list:
            ...
            fio = "%s --name=MYJOB-%s --filename='%s'" % (
                fio, short_disk, disk)
            global_disk_list += '%s,' % short_disk
        global_disk_list = global_disk_list.rstrip(',')
        sys.stderr.write(
            'Benchmarking storage {} for {} seconds in {} mode with blocksize={} (engine={})\n'.format(
                global_disk_list, time, mode, io_size, engine))
        fio_cmd = subprocess.Popen(fio, shell=True, stdout=subprocess.PIPE)
        current_disk = ''
        for line in fio_cmd.stdout:
            if ('MYJOB-' in line) and ('pid=' in line):
                # MYJOB-sda: (groupid=0, jobs=1): err= 0: pid=23652: Mon Sep  9
                # 16:21:42 2013
                current_disk = re.search(
                    'MYJOB-(.*): \(groupid', line).group(1)
                continue
            if ("read: IOPS=" in line) or ("write: IOPS=" in line):
                # read: IOPS=225, BW=225MiB/s (236MB/s)(255MiB/1133msec)\n'
                if (len(disks_list) > 1):
                    mode_str = '{}_simultaneous_{}_{}'.format(
                        engine, mode, io_size)
                else:
                    mode_str = '{}_standalone_{}_{}'.format(
                        engine, mode, io_size)
